    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if action == 'search_report':
                ...
                purchase = float(purchase.aggregate(resp=Coalesce(Sum('subtotal'), 0.00, output_field=FloatField())).get('resp'))

                sale = Sale.objects.all()
                if len(start_date) and len(end_date):
                    sale = sale.filter(date_joined__range=[start_date, end_date])
                sale = float(sale.aggregate(resp=Coalesce(Sum('total'), 0.00, output_field=FloatField())).get('resp'))

                expenses = Expenses.objects.all()
                if len(start_date) and len(end_date):
                    expenses = expenses.filter(date_joined__range=[start_date, end_date])
                expenses = float(expenses.aggregate(resp=Coalesce(Sum('valor'), 0.00, output_field=FloatField())).get('resp'))
